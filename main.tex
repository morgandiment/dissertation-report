...
\maketitle{%
  ...
  Battleship Solitaire is a determinnistic logic puzzle whose decision problem is NP-Complete, making it a useful testbed for advanced combinatorial optimisation techniques.
  This dissertation compares two Integer Linear Programming (ILP) formulations of the puzzle, originally proposed by Meufells and den Hertog, an d quantifies their behaviour under exponential scaling.
  A Cell-based model represents the grid as a per-cell state matrix, using shape-transition and adjacency contraints.
  A Ship-based model uses candidate generation to find valid placements and resolves geometric conflicts with Edge-Based Set Packing. 
  A third variant adds four classes of valid inequalities to the Cell model, to study polyhedral tightening.
  
  To benchmark these formulations, an automated framework was developed in Python and Gurobi. A CSPLib Prolog parser, a procedural puzzle generator that produces feasible instances on grids up to $30 \times 30$, and a thread-safe CustomTkinter interface for interactive solving were engineered.
  Across 150 generated instances and the standard $10 \times 10$ CSPLib dataset, the results show the Ship model dominates, solving $30 \times 30$ zero-hint instances in under two seconds. The Cell model required up to twenty-two seconds for the same configurations. 
  Branch-and-Bound node counts remained near unity for all models, isolating the Cell-model's bottleneck to matrxi density rather than search-tree depth.
  A constraint ablation study further showed that valud inequalities are scale-dependent. At 900 cells, they reduce the solve time by approximately two seconds, but introduce overhead at smaller scales.
...
I would also to thank my close ones who have supported me during this project. The behind-the-scenes help did more than you'll ever know. 
...
The standard datasets utilised for the baseline validation were sourced from the Constraint Satisfaction Problem Library \citep{csplib:prob014}. The specific Solitaire Battleship (Problem 014) data is available at: \url{https://www.csplib.org/Problems/prob014/data/}.
...
In accordance with the CM3203 project guidelines, I declare the use of AI assistive technologies during the development of this project. Specifically, Google's Gemini Large Language Model \citep{gemini} was utilised as an assistive programming and typesetting tool. Its primary applications included:
\begin{itemize}
  \item \textbf{Troubleshooting Platform Issues:} Assisting in identifying the root cause of the macOS-specific \verb|NSInternalInconsistencyException| thread-safety crashes within the CustomTkinter Graphical User Interface.
  \item \textbf{Syntax Generation:} Aiding with the creation of boilerplate Regular Expressions (Regex) for the CSPLib parser.
  \item \textbf{Typesetting:} Formatting manually derived mathematical equations into the correct LaTeX syntax.
\end{itemize}

The use of this tool was highly appropriate for the project. It functioned as an advanced, interactive reference manual, which accelerated debugging and formatting tasks. 

The core intellectual property of this project, such as the system architecture, ILP constraint definitions and experiment methodology, was entirely authored by myself.

To mitigate the inherent risk of AI use and the hallucinations it can have, all AI-generated suggestions were critically reviewed, manually integrated and tested fully to ensure mathematical and programmatic validity.
