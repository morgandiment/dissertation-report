...
\maketitle{%
  ...
  To benchmark these formulations, an automated framework was developed in Python and Gurobi. A \gls{csplib} Prolog parser, a procedural puzzle generator that produces feasible instances on grids up to $30 \times 30$, and a thread-safe CustomTkinter interface for interactive solving were engineered.
...
In accordance with the CM3203 project guidelines, I declare the use of AI assistive technologies during the development of this project. Specifically, Google's Gemini Large Language Model \citep{gemini} was involved in three narrow capacities, none of which involved authoring the project's mathematical, architectural or analytical content:
\begin{itemize}
  ...
  \item \textbf{Boilerplate Regex Syntax:} The \gls{csplib} Prolog parser uses a single \verb|re.compile(...)| invocation (Section~\ref{subsec:data-structures-parsing}). Gemini provided the initial \verb|\s*\(\s*(\d+)\s*,| escape pattern; the five capture groups, the \verb|re.DOTALL| flag, and the secondary hint-extraction regex were authored manually after testing the pattern against \verb|data/csplib.pl|.
...
The system architecture (Section~\ref{sec:system-architecture}), the formal mathematical specifications of both \gls{ilp} models (Sections~\ref{sec:math-formulation-cell} and~\ref{sec:math-formulation-ship}), the four valid inequalities (Section~\ref{sec:valid_inequalities}), the procedural puzzle generator (Section~\ref{subsec:procedural-puzzle-gen-algo}), the evaluation methodology (Chapter~\ref{ch4}) and all of the dissertation prose were authored by myself without AI assistance.

To mitigate the risk of hallucinated output, every AI suggestion was reviewed before integration, tested for behavioural correctness against the unit tests in \verb|tests/| (which validate both ILP formulations against feasibility-checked reference instances via \verb|BattleshipBoard.is_valid_solution|), and cross-checked against either the source paper (for mathematical content) or the CSPLib data file (for parser content). No suggestion was incorporated unmodified.
