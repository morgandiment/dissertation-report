...
The ship-based solver works by generating every valid ship placement upfront and ask the solver to pick the correct combination of ships from the selection, instead of working cell-by-cell. The constraint equations are from Meuffels \citep{ilp-paper}.
...
To ensure the solvers were mathematically sound and could replicate known academic benchmarks, the system was engineered to ingest data from the Constraint Satisfaction Problem Library (CSPLib), specifically Problem 014 (Solitare Battleships).
