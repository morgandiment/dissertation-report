...
\begin{itemize}
  \item \textbf{Automated Evaluation Pipeline and Generator:} The development of a robust Python framework capable of parsing standard CSPLib Prolog datasets, alongside a dynamic puzzle generator designed to create mathematically valid instances up to 30x30 in size for scalability analysis.
  ...
  \item \textbf{Constraint Impact Analysis and Tuning:} A Constraint Impact Analysis revealed the 'Overhead vs. Pruning' trade-off, demonstrating that while explicit corner-banning reduced solve times on hard instances by pruning dead-end search branches, redundant parity constraints introduced matrix bloat affected performance in certain conditions.
  \item \textbf{Scalability and Performance Discovery:} Conclusive evidence demonstrating that the Ship-based Set Packing formulation strictly dominates the Cell-based approach. The Ship model maintains an immense speed advantage, efficiently solving 30x30 zero-hint instances in under 2 seconds, where the Cell model severely weakens, taking upwards of 25 seconds.
\end{itemize}
